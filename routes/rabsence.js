...
router.post("/", function(req, res, next) {
  ...
  model.Absence.findAll({
    where: { AbsentDate: req.body.AbsentDate, EmployeeID: req.body.EmployeeID }
  })
    .then(result => {
      if (result.length > 0) {
        //update
                ...
                model.Employee.findAll({
                  ...
                  let objEmail = {
                    Message:
                      "<b>Dh Bapak/Ibu " +
                      boss.FullName +
                      ",</b><br/><br/>Absen karyawan atas nama " +
                      emp.FullName +
                      " membutuhkan approval Anda, karena ada koreksi/ketidaksesuaian waktu atau lokasi.<br/>Mohon segera ditindaklanjut<br/><br/><br/>" +
                      buttonLink +
                      signature,
...
router.put("/:empid", function(req, res, next) {
  let objPut = req.body;
  model.Absence.update(objPut, { where: { Id: objPut.Id } }).then(result => {
    messageBuilder(req.params.empid, cb => {
      ...
      switch (objPut.Status) {
        case 0:
          url +=
            config.baseUrl + "/#/main/admin/set-absence?idabs=" + objPut.Id;
          buttonLink +=
            "<br/><a href='" +
            url +
            "' style='width: 300px; padding: 12px; cursor: pointer; box-shadow: 1px 1px 3px #000; -webkit-box-shadow: 1px 1px 3px #000; -moz-box-shadow: 1px 1px 3px #000; font-weight: bold; background: #203072; color: #fff; border-radius: 10px;text-decoration:none;font-family:verdana'>" +
          ...
          message +=
            emp.FullName +
            "<br/><br/>Pemberitahuan terkait data absen Anda telah ";
          receiver = [
            boss.EmailPrivate,
            boss.EmailOffice,
            emp.EmailPrivate,
          ...
          message +=
            "<b>DITOLAK</b>" + " oleh Bpk/Ibu " + boss.FullName + buttonLink;
          break;
        case 2:
          url +=
            config.baseUrl +
          ...
          message +=
            boss.FullName +
            "<br/><br/>karyawan Anda atas nama " +
            emp.FullName +
            " telah melakukan ";
          receiver = [boss.EmailPrivate, boss.EmailOffice];
          message +=
            "<b>KOREKSI</b> data Absen, mohon segera ditindaklanjuti." +
            buttonLink;
          break;
        case 3:
          url +=
          ...
          message +=
            "<b>DISETUJUI</b>" + " oleh Bpk/Ibu " + boss.FullName + buttonLink;
          break;
      }
      message +=
        " pada tanggal " + moment().format("YYYY-MM-DD HH:mm:ss") + signature;
      let objEmail = {
        Message: message,
        Sender: "ess-app@acset.co",
        Receiver: receiver,
...
router.get("/lembur/last", function(req, res, next) {
  let empId = req.query.empid;
  model.AbsenceLembur.max("Id", { where: { EmployeeID: empId } }).then(
    result => {
      if (result) {
        model.AbsenceLembur.findAll({ where: { Id: result } }).then(
          maxAbsen => {
            res.json(maxAbsen);
          }
        );
...
router.post("/lembur/maintain/cr", function(req, res, next) {
  model.uv_EmployeeAbsenceLembur
    .findAll({
      where: req.body,
      order: [["AbsentDate", "DESC"]]
    })
...
router.post("/lembur/cr/:page/:pagesize", function(req, res, next) {
  ...
  const offset = req.params.page * req.params.pagesize;
  const limit = offset + pagesize;
  model.AbsenceLembur.findAll({ limit, offset, where: req.body }).then(
    result => {
      res.json(result);
    }
  );
  ...
  model.AbsenceLembur.findAll({
    ...
    .then(result => {
      if (result.length > 0) {
        //update
        let objAbsen = result[0];
        model.AbsenceLembur.update(req.body, {
          where: {
            AbsentDate: req.body.AbsentDate,
            EmployeeID: req.body.EmployeeID
          }
            ...
            model.Employee.findAll({
              ...
              let objEmail = {
                Message:
                  ...
                  ",</b><br/><br/>Absen karyawan atas nama " +
                  emp.FullName +
                  " membutuhkan approval Anda, karena ada koreksi/ketidaksesuaian absen <b>lembur</b>.<br/>Mohon segera ditindaklanjut<br/><br/><br/>" +
                  buttonLink +
                  signature,
                Sender: "ess-app@acset.co",
                Receiver: [boss.EmailPrivate, boss.EmailOffice],
        ...
        //insert
        model.AbsenceLembur.create(req.body)
          .then(ins => {
            res.json(ins);
          })
          .catch(err => {
...
router.put("/lembur/:empid", function(req, res, next) {
  let objPut = req.body;
  model.AbsenceLembur.update(objPut, { where: { Id: objPut.Id } }).then(
    result => {
      messageBuilder(req.params.empid, cb => {
        let emp = cb;
        let boss = cb.Boss;
...
router.post("/sapreport", function(req, res, next) {
  if (req.body) {
    model.sequelize
      .query("EXEC up_sapAbsence :Awal,:Akhir", {
