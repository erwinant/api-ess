...
//             console.log(eachAppr);
//         });
//     })
// });
model.Approval.findAll({
    where: { DocStatus: 'INIT' },
    raw: true,
    nest: true,
    include: [
        {
    ...
    apprHeader.forEach(eachAppr => {
        model.ApprovalStep.findAll({
            where: { AppType: eachAppr.AppType, StepNumber: eachAppr.StepCurrent + 1 },
            raw: true
        }).then((appStep) => {
            //check direct email / organization
            appStep = appStep[0];
            if (appStep.Email) // direct mail
            {
                //get the person by email (let say the boss)
                model.Employee.findAll({
                    where: {
                        [Op.or]: [{ EmailOffice: appStep.Email }, { EmailPrivate: appStep.Email }]
                    },
                    raw: true
                }).then((theBoss) => {
                    ...
                    model.ApprovalDetail.create(apprDet).then((insDet) => {
                        model.Approval.update({ DocStatus: "MENUNGGU APPROVAL " + appStep.Email }, { where: { Id: eachAppr.Id } }).then((up) => {

                            let emp = eachAppr.Employee;
                            let url = config.baseUrl + "/#/main/admin/set-absence?idabs=" + eachAppr.Id + "&apabs=1&lembur=1";
                            ...
                            let objEmail = {
                                Message: "<b>Dh Bapak/Ibu,</b><br/><br/>Permintaan karyawan atas nama " + emp.FullName + " membutuhkan approval Anda<br/>Mohon segera ditindaklanjut<br/><br/><br/>" + buttonLink + signature,
                                Sender: "ess-app@acset.co",
                                Receiver: [appStep.Email],
                                Subject: "[ess-app] Approval " + eachAppr.AppType + " - " + emp.FullName,
                                Cc: ""
                            }
            ...
            } else { //go organization
                if (eachAppr.Employee.Boss.Id == null) //tidak ada boss
                { console.log("No Boss") }
                else {
                    let apprDet = {
                        RowStatus: 1,
                    ...
                    model.ApprovalDetail.create(apprDet).then((insDet) => {
                        ...
                        model.Approval.update({ DocStatus: "MENUNGGU APPROVAL " + eachAppr.Employee.Boss.EmailOffice }, { where: { Id: eachAppr.Id } }).then((up) => {
                            let emp = eachAppr.Employee;
                            ...
                            let signature = "<br/><br/>Mohon tidak membalas email ini. Email otomatis yang dikirimkan ess-app.<br/>"
                            let objEmail = {
                                Message: "<b>Dh Bapak/Ibu " + boss.FullName + ",</b><br/><br/>Permintaan karyawan atas nama " + emp.FullName + " membutuhkan approval Anda<br/>Mohon segera ditindaklanjut<br/><br/><br/>" + buttonLink + signature,
                                Sender: "ess-app@acset.co",
