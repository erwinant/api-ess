...
router.post('/cr', function (req, res, next) {
    ...
    model.Employee.findAll({
        ...
        include: [
            ...
            { model: model.EmployeeTrainingTemp, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeFamilyTemp, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeEduTemp, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeAttachmentTemp, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeTraining, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeFamily, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeEdu, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeAttachment, where: { RowStatus: 1 }, required: false }
        ],
    }).then((result) => {
        res.json(result);
    })
...
router.put('/attachtemp', function (req, res, next) {
    let obj = req.body;
    model.EmployeeAttachmentTemp.update(obj, { where: { Filename: obj.Filename } }).then((result) => {
        res.json(result);
    })
});
router.put('/attach', function (req, res, next) {
    let obj = req.body;
    model.EmployeeAttachment.update(obj, { where: { Id: req.body.Id } }).then((result) => {
        res.json(result);
    })
});
...
router.put('/edutemp', function (req, res, next) {
    let obj = req.body;
    model.EmployeeEduTemp.update(obj, { where: { Id: req.body.Id } }).then((result) => {
        res.json(result);
    })
});
...
router.put('/trntemp', function (req, res, next) {
    let obj = req.body;
    model.EmployeeTrainingTemp.update(obj, { where: { Id: req.body.Id } }).then((result) => {
        res.json(result);
    })
});
    ...
    if (req.params.idabs) {
        model.EmployeeTemp.findAll({
            ...
            people().then(peopleRes => {
                let objEmail = {
                    Message: "Konfirmasi data diri "+peopleRes.Employee.FullName+" ditolak. <br/>. Mohon hubungi HC terkait.",
                    Sender: "ess-app@acset.co",
                    Receiver: [peopleRes.Admin.EmailOffice, peopleRes.Employee.EmailOffice],
                    Subject: "[ess-app] Perubahan Data Diri ditolak - " + peopleRes.Employee.FullName,
                    Cc: [peopleRes.Admin.EmailPrivate, peopleRes.Employee.EmailPrivate],
                }
                commonFunction.sendEmail(objEmail, cb => {
                    console.log(cb);
                })
                commonFunction.saveMessage(objEmail, peopleRes.Employee, peopleRes.Admin, cb => {
                    console.log(cb);
                })
                commonFunction.saveMessage(objEmail, peopleRes.Admin, peopleRes.Employee, cb => {
                    console.log(cb);
                })
            });

            model.EmployeeTemp.destroy({ where: { EmployeeID: req.params.idabs } }).then((del) => {
                if (del) {
...
router.get('/temp/approve/:idabs', function (req, res, next) {
    if (req.params.idabs) {
        model.EmployeeTemp.findAll({
            ...
            //moving to real one
            model.Employee.update(editedObj, { where: { Id: req.params.idabs } })
                .then((result) => {
                    ...
                    people().then(peopleRes => {
                        let objEmail = {
                            Message: "Konfirmasi perubahan data diri "+peopleRes.Employee.FullName+" telah disetujui.",
                            Sender: "ess-app@acset.co",
                            Receiver: [peopleRes.Admin.EmailOffice, peopleRes.Employee.EmailOffice],
                            Subject: "[ess-app] Perubahan Data Diri - " + peopleRes.Employee.FullName,
                            Cc: [peopleRes.Admin.EmailPrivate, peopleRes.Employee.EmailPrivate],
                        }
                        commonFunction.sendEmail(objEmail, cb => {
                            console.log(cb);
                    ...
                    model.EmployeeTemp.destroy({ where: { EmployeeID: req.params.idabs } })
                        .then((del) => {
...
router.post('/temp', function (req, res, next) {
    model.Employee.findAll({
        ...
        commonFunction.getEmployee(null, '80000000', (admin) => {
            ...
            let urlCek = config.baseUrl + "/#/main/reg-user/profile-main/" + objEmp.NRP;
            let urlApprove = config.apiUrl + "employee/temp/approve/" + objEmp.Id;
            let urlReject = config.apiUrl + "employee/temp/reject/" + objEmp.Id;
            let buttonCekLink = "<a href='" + urlCek + "' style='width: 300px; padding: 12px; cursor: pointer; box-shadow: 1px 1px 3px #000; -webkit-box-shadow: 1px 1px 3px #000; -moz-box-shadow: 1px 1px 3px #000; font-weight: bold; background: #203072; color: #fff; border-radius: 10px;text-decoration:none;font-family:verdana'>" +
                "<b>Lihat profil saat ini</b>" +
                "</a>&nbsp;<br/><br/>";
            let buttonApproveLink = "<a href='" + urlApprove + "' style='width: 300px; padding: 12px; cursor: pointer; box-shadow: 1px 1px 3px #000; -webkit-box-shadow: 1px 1px 3px #000; -moz-box-shadow: 1px 1px 3px #000; font-weight: bold; background: #03fc8c; color: #fff; border-radius: 10px;text-decoration:none;font-family:verdana'>" +
                "<b>Setujui</b>" +
                "</a>&nbsp;";
            let buttonRejectLink = "<a href='" + urlReject + "' style='width: 300px; padding: 12px; cursor: pointer; box-shadow: 1px 1px 3px #000; -webkit-box-shadow: 1px 1px 3px #000; -moz-box-shadow: 1px 1px 3px #000; font-weight: bold; background: #fc3903; color: #fff; border-radius: 10px;text-decoration:none;font-family:verdana'>" +
                "<b>Tolak perubahan</b>" +
                "</a><br/><br/>";
            let propA = "Permohonan perubahan data : <br/>";
            Object.keys(obj).forEach(key => {
                if (objEmp[key] !== obj[key]) {
                    propA += key + " : Berubah dari <b>" + objEmp[key] + "</b> menjadi <b>" + obj[key] + "</b><br/>"
                }
            });
            ...
            let message = "<b>Dh Bapak/Ibu HCD Admin,</b><br/><br/>Karyawan atas nama " + objEmp.FullName +
                " melakukan perubahan data diri.<br/>Mohon segera ditindaklanjut<br/><br/><br/>" + propA +
                "<br/><br/>" + buttonCekLink + buttonApproveLink + "&nbsp;&nbsp;" + buttonRejectLink + signature;
    ...
    model.EmployeeTemp.findAll({
        ...
        if (emp.length > 0) {
            let objEmp = emp[0];
            model.EmployeeTemp.update(obj, { where: { Id: objEmp.Id } }).then((result) => {
                res.json(result);
            })
        } else {
    ...
    model.Employee.update(obj, { where: { Id: req.body.Id } }).then((result) => {
        model.Employee.findAll({
            ...
            model.Employee.findAll({
                ...
                Object.keys(obj).forEach(key => { propA += key + " : " + objEmp[key] + "<br/>" });

                let propB = "Perubahan data awal: <br/>";
                Object.keys(obj).forEach(key => { propB += key + " : " + obj[key] + "<br/>" });

                let signature = "<br/><br/>Mohon tidak membalas email ini. Email otomatis yang dikirimkan ess-app.<br/>"
                let message = "<b>Dh Bapak/Ibu HCD Admin,</b><br/><br/>Karyawan atas nama " + objEmp.FullName + " melakukan perubahan data diri.<br/>Mohon segera ditindaklanjut<br/><br/><br/>" + propA + "<br/>" + propB + "<br/>" + buttonLink + signature;
...
router.post('/quickprofile', function (req, res, next) {
    model.uv_EmployeeQuickProfile.findAll({ where: req.body }).then(result => {
        res.json(result);
    });
});
router.post('/quickprofile-simple', function (req, res, next) {
    model.uv_EmployeeQuickProfile.findAll({ where: req.body, attributes: ['FullName', 'EmployeeId', 'Username'] }).then(result => {
        res.json(result);
    });
});
router.post('/quickprofile/:page/:pagesize', function (req, res, next) {
    ...
    model.uv_EmployeeQuickProfile.findAll({ pagesize, page, where: req.body }).then((result) => {
        res.json(result);
    })
});
...
router.post('/approvechange/:type/:nrp', function (req, res, next) {
    model.Employee.findAll({
        ...
        include: [
            { model: model.EmployeeTemp, where: { RowStatus: 1 }, required: false, attributes: { exclude: ['Id'] } },
            { model: model.EmployeeTrainingTemp, where: { RowStatus: 1 }, required: false, attributes: { exclude: ['Id'] } },
            { model: model.EmployeeFamilyTemp, where: { RowStatus: 1 }, required: false, attributes: { exclude: ['Id'] } },
            { model: model.EmployeeEduTemp, where: { RowStatus: 1 }, required: false, attributes: { exclude: ['Id'] } },
            { model: model.EmployeeAttachmentTemp, where: { RowStatus: 1 }, required: false, attributes: { exclude: ['Id'] } },
            { model: model.EmployeeTraining, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeFamily, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeEdu, where: { RowStatus: 1 }, required: false },
            { model: model.EmployeeAttachment, where: { RowStatus: 1 }, required: false }
        ...
        if (req.params.type === "fam" && objEmp.EmployeeFamilyTemps.length) {
            ...
            model.EmployeeFamily.bulkCreate(convertObj, { returning: true }).then((bulk) => {
                if (bulk.length) {
                    model.EmployeeFamilyTemp.destroy({ where: { EmployeeID: objEmp.Id } }).then((del) => {
            ...
            model.EmployeeEdu.bulkCreate(convertObj, { returning: true }).then((bulk) => {
                if (bulk.length) {
                    model.EmployeeEduTemp.destroy({ where: { EmployeeID: objEmp.Id } }).then((del) => {
                        res.json({ message: true });
                    })
                }
            })
            ...
            model.EmployeeTraining.bulkCreate(convertObj, { returning: true }).then((bulk) => {
                if (bulk.length) {
                    model.EmployeeTrainingTemp.destroy({ where: { EmployeeID: objEmp.Id } }).then((del) => {
                        res.json({ message: true });
                    })
                }
            })
        ...
        if (req.files.attach) {
            try {
                ...
                myfile.mv(directoryEmployee + myfile.name, function (err) {
                    if (err)
                        return res.status(500).send(err);
                    res.status(200).send({ "filename": myfile.name, "systemname": originalName });
                });
            } catch (e) {
                res.status(500).send({ "filename": e });
            }
