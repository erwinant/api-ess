...
router.put("/", function(req, res, next) {
  let obj = req.body;
  model.Account.update(obj, { where: { Username: obj.Username } }).then(
    result => {
      res.json(result);
    }
  );
...
router.post("/register", function(req, res, next) {
  if (req.body) {
    ...
    model.sequelize
      .query(
        "EXEC sp_AccountGenerator :Password,:Username,:FullName,:Gender,:Role,:EmailPrivate,:DivisionID,:DepartmentID,:OrganizationLevelID,:DirectReportID,:ClockIn,:ClockOut,:CreateBy",
        { replacements: obj, type: Sequelize.QueryTypes.SELECT }
      )
      .then(result => {
        if (result.length) {
          ...
          let objEmail = {
            Message:
              "<b>Selamat Bergabung " +
              obj.FullName +
              "</b>, <br/><br/>Pendaftaran akun ESS Anda telah berhasil,<br/>Mohon segera lengkapi profil Anda di " +
              config.baseUrl +
              ". <br/> Login dengan menggunakan : <br/><table><tr><th>Username</th><th>Password</th></tr><tr><td>" +
              obj.Username +
              "</td><td>" +
              obj.Password +
              "</td></table>" +
          ...
          model.sequelize
            .query("SELECT Id FROM Employee WHERE NRP = $1", {
            ...
            .then(res => {
              ...
              if (obj.AttendanceLocations) {
                let EmployeeLocations = obj.AttendanceLocations.map(loc => {
                  let newEmployeeLoc = {
                    EmployeeID: res[0].Id,
                    LocationID: loc.LocationID,
                    CreateBy: loc.CreateBy
                  };
                  return newEmployeeLoc;
                });
                model.EmployeeLocation.bulkCreate(EmployeeLocations);
              }
            });
        } else {
...
router.post("/resetpwd", function(req, res, next) {
  if (req.body) {
    ...
    obj.Password = uuidv1()
      ...
      .substring(0, 6);
    model.sequelize
      .query("EXEC sp_AccountUpdate :Username,:Password,:UpdateBy", {
        replacements: obj,
      ...
      .then(result => {
        if (result.length) {
          ...
          let objEmail = {
            Message:
              "<b>Permintaan reset password oleh " +
              obj.FullName +
              "</b>, <br/><br/>Mohon segera login di " +
              config.baseUrl +
              ". <br/> Login dengan menggunakan : <br/><table><tr><th>Username</th><th>Password</th></tr><tr><td>" +
              obj.Username +
              "</td><td>" +
  ...
  if (req.body) {
    model.sequelize
      ...
      .then(result => {
        if (result.length > 0) {
          model.sequelize
            .query("EXEC sp_AccountUpdate :Username,:NewPassword1, :UpdateBy", {
              replacements: req.body,
              type: Sequelize.QueryTypes.SELECT
            })
            .then(up => {
